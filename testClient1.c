...
=======
서버 -> 클라이언트 : 사용자 위치, 현재 자동차의 위치 
/// 클라이언트 -> 서버 : 시리얼 넘버, 상태변수
>>>>>>> 2cef0acf5b14a836d4b85c3e1ae221a6a86b0884
...
서버 -> 클라이언트 : 자동차1 위치, 자동차2 위치 /// 클라이언트 -> 서버 : 시리얼 넘버, 상태변수
=======
[R(Run):그런 다음에 서버에서 모든 거리값들을 비교한 다음에 서버에서 거리 반환한 모든 클라이언트들에게 선택된 클라이언트의 고유코드를 반환한다.]
서버 -> 클라이언트 : 모든 차들에게 선택된 시리얼 넘버를 보낸다 
/// 클라이언트 -> 서버 : 시리얼 넘버, 상태변수

[W():핸드폰으로부터 사용]
서버 -> 클라이언트 : 핸드폰 입력 여부 확인 
/// 클라이언트 -> 서버 : 시리얼 넘버, 상태변수

[A(Activate):만약 선택되면 선택된 차는 모터를 활성화 시키고 서버로부터 지속적으로 자신의 위치와 다른 자동차의 위치를 받는다.]
서버 -> 클라이언트 : 자동차1 위치, 자동차2 위치, 사용자 도착지점 
/// 클라이언트 -> 서버 : 시리얼 넘버, 상태변수
